...
function Home() {
  ...
  const loadData = async () => {
    try {
      let response = await fetch("https://sarthak-sir-repo.onrender.com/api/displayData", {
        method: 'GET',
        headers: {
...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://sarthak-sir-repo.onrender.com/api/displayData');
        if (!response.ok) {
          throw new Error(`Error: ${response.status} - ${response.statusText}`);
        }
    ...
    return (
      <div className='custom-grid'>
        {toolsData.map((data, index) => (
          <Card
            key={data.index} // Use a unique key
            prize={data.price}
            title={data.name}
            info={data.description}
            imageSrc={data.img}
